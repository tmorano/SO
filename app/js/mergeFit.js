sistemasOperacionais.factory('MergeFitService', function (MemoryHelper,$interval,$timeout) {
  var mergeFit = {};
  var header;
  var blockCounter = 0;
  ...
  mergeFit.encerrarProcesso = function(processo,block,index){
    if(!activate){
      time = $timeout(function(){
        mergeFit.config.arrayOfProcessMemory.series = mergeFit.memory.blocks;
      },100);
      activate = true;
    }
    this.memory.size += processo.memory;
    this.merge(processo,this.memory.blocks[0],0);
    if(this.memory.blocks.length == 1){
      $timeout.cancel(time);
    }
  };
